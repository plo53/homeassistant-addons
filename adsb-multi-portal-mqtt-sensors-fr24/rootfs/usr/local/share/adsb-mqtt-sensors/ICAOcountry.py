...
unassigned_range = { "country": "Unassigned", "flag_image": "null" }


def initICAOCountries():
    countries={}
    for i in range( len(ICAO_Ranges) ):
        countries[ICAO_Ranges[i]["country"]] = 0
    return countries

def findICAORange(icao):
    hexa = "0x"
    hexa += icao
    hexa = int(hexa,16)
    for i in range( len(ICAO_Ranges) ):
        if ( hexa >= int(ICAO_Ranges[i]["start"]) and hexa <= int(ICAO_Ranges[i]["end"]) ):
            return ICAO_Ranges[i]
    return unassigned_range

def findICAOCountry(icao):
    hexa = "0x"
    hexa += icao
    hexa = int(hexa,16)
    for i in range( len(ICAO_Ranges) ):
        if ( hexa >= int(ICAO_Ranges[i]["start"]) and hexa <= int(ICAO_Ranges[i]["end"]) ):
            return ICAO_Ranges[i]["country"]
    return None
